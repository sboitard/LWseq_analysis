...
mydir=args[1]
chro=as.integer(args[2])

# res LWD
res_LWD = fread(paste(mydir,'/results/time/snp20_auto_cr_LWD_SL_xi3.txt',sep=''), head=T, select=c('chr','pos','pvalue','lindley','th')) %>% filter(chr==chro)
res_LWD = res_LWD %>% mutate(signif=(lindley>=th)) %>% select(chr,pos,pvalue,signif) %>% mutate(test='LWD')

# res LWS
res_LWS = fread(paste(mydir,'/results/time/snp20_auto_cr_LWS_SL_xi3.txt',sep=''), head=T, select=c('chr','pos','pvalue','lindley','th')) %>% filter(chr==chro)
...
hapflk_signif=hapflk_signif %>% mutate(rs=paste(chr,pos,sep=':')) %>% filter(chr==chro)

# res hapflk
flk=fread(paste(mydir,'/results/FLK/snp20_auto_cr_maf10_2pop.hapflk_sc',sep=''),header=T,select=c('rs','chr','pos','pvalue'))
flk = flk %>% filter(!is.na(pvalue)) %>% filter(chr==chro)
...
# add fictive points to keep all categories
fake=res[1:2,]
fake$pos=-10
fake$test=c('LWD signif','LWS signif')
res=rbind(res,fake)
...
p=ggplot(res,aes(x=pos,y=log10_pval,colour=test))+geom_point()+theme_bw() +scale_colour_manual(name='Test',values = brewer.pal(6,'Paired')) + xlab('position (Mb)') + ylab('log10(pvalue)')+xlim(0,max(res$pos))
p=p+geom_point(aes(x=pos,y=star,colour=test),shape=17)
ggsave(paste(mydir,'/results/LW_summary_2pop_chro',chro,'_plot.png',sep=''), plot = p, width = 6, height = 3)
